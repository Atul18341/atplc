  ...
  const generateCerifiacte = async () => {
    try {
      ...
      try {
        ...
        const existingSignBytes = await fetch(signUrl).then((res) =>
          res.arrayBuffer()
        );
        const existingQRBytes = await fetch(qrUrl).then((res) =>
          res.arrayBuffer()
        );
        ...
        const romanFont = await pdfDoc.embedStandardFont(
        ...
        if (!College_Name) {
          setLoading(false);
          return toast.error(
            "Please update your college name in profile section to get certificate"
          );
        }
    ...
    <section className="certificate-section">
      ...
      <div className="section-body">
        <div className="certificate-criteria">
          <p>
            You must submit at least <span>75%</span> of tasks to be eligible
            for the training certificate.
          </p>
          <p>
            Once you meet the criteria, a download button for your training
        ...
        <div className="current-percentage">
          <p>
            ...
            <span
              className={`${
                (completedTask / totalTask) * 100 >= 75 ? "success" : "danger"
              }`}
            >
